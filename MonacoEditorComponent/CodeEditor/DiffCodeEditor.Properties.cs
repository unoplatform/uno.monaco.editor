...
{
    /* OriginalText: The original (left-hand) document -- the "before" side.
     * ModifiedText: The modified (right-hand) document -- the "after" side, and the editable one.
     * OriginalLanguage: Syntax language of the original side. Unset follows CodeLanguage.
     * OriginalEditable: Unlocks the original side. A pass-through for DiffOptions.OriginalEditable.
     * DiffOptions: Diff-specific options -- side-by-side vs inline, whitespace handling, the diff
     *      algorithm, collapsing of unchanged regions. Distinct from the inherited Options, which
     *      configures the modified side's underlying editor.
     */

    #region DependencyProperty: OriginalText

    /// <summary>Identifies the <see cref="OriginalText"/> dependency property.</summary>
    public static DependencyProperty OriginalTextProperty { get; } = DependencyProperty.Register(
        nameof(OriginalText),
        typeof(string),
    ...
    #endregion
    #region DependencyProperty: ModifiedText

    /// <summary>Identifies the <see cref="ModifiedText"/> dependency property.</summary>
    public static DependencyProperty ModifiedTextProperty { get; } = DependencyProperty.Register(
        nameof(ModifiedText),
        typeof(string),
        typeof(DiffCodeEditor),
        new PropertyMetadata(string.Empty, OnModifiedTextChanged));
    ...
    #endregion
    #region DependencyProperty: OriginalLanguage

    /// <summary>Identifies the <see cref="OriginalLanguage"/> dependency property.</summary>
    public static DependencyProperty OriginalLanguageProperty { get; } = DependencyProperty.Register(
        nameof(OriginalLanguage),
        typeof(string),
        typeof(DiffCodeEditor),
        new PropertyMetadata(default(string), OnOriginalLanguageChanged));
    ...
    #endregion
    #region DependencyProperty: OriginalEditable
    ...
    public static DependencyProperty OriginalEditableProperty { get; } = DependencyProperty.Register(
        ...
        typeof(bool),
        typeof(DiffCodeEditor),
        new PropertyMetadata(default(bool), OnOriginalEditableChanged));
    ...
    private static void OnOriginalTextChanged(DependencyObject control, DependencyPropertyChangedEventArgs e) => ((DiffCodeEditor)control).OnOriginalTextChanged(e);
    private static void OnModifiedTextChanged(DependencyObject control, DependencyPropertyChangedEventArgs e) => ((DiffCodeEditor)control).OnModifiedTextChanged(e);
    private static void OnOriginalLanguageChanged(DependencyObject control, DependencyPropertyChangedEventArgs e) => ((DiffCodeEditor)control).OnOriginalLanguageChanged(e);
    private static void OnOriginalEditableChanged(DependencyObject control, DependencyPropertyChangedEventArgs e) => ((DiffCodeEditor)control).OnOriginalEditableChanged(e);
    private static void OnDiffOptionsChanged(DependencyObject control, DependencyPropertyChangedEventArgs e) => ((DiffCodeEditor)control).OnDiffOptionsChanged(e);
}
