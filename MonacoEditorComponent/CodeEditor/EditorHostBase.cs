...
namespace Monaco
{
    ...
    /// <summary>
    /// Shared host for a cross-platform Uno Platform wrapper around the
    ...
    /// </summary>
    /// <remarks>
    /// Owns everything that is identical across editor flavors: presenter creation and
    /// reuse, the initialization handshake and its recovery paths, the bridge helpers,
    /// theming, decorations, markers, and script invocation. Derived controls supply only
    /// the bootstrap entry point and their own text properties -- see
    /// <see cref="BootstrapFunctionName"/>, <see cref="PrimaryText"/>, and
    /// <see cref="BuildInitialStateJson"/>.
    /// <para>
    /// Use <see cref="CodeEditor"/> for a single editable document, or
    /// <c>DiffCodeEditor</c> for a two-sided comparison view.
    /// </para>
    /// </remarks>
    ...
    public abstract partial class EditorHostBase : Control, INotifyPropertyChanged, IDisposable
    {
        ...
        /// Monaco instance.
        /// </summary>
        protected virtual string BootstrapFunctionName => "createMonacoEditor";
        ...
        /// Monaco during initialization and embedded in the desktop initial-state payload.
        /// </summary>
        protected abstract string? PrimaryText { get; }

        /// <summary>
        /// Registers the JavaScript to C# bridge callbacks for this control. The base
        /// implementation wires the "Loaded" handshake; derived controls that override this
        /// must call the base implementation. Declared <c>private protected</c> because
        /// <c>IParentAccessor</c> is internal to this assembly.
        /// </summary>
        /// <param name="accessor">The parent accessor bridge, or <see langword="null"/> when unavailable.</param>
        private protected virtual void RegisterBridgeCallbacks(IParentAccessor? accessor)
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private async Task DeferredTeardownAsync(CancellationToken ct)
        {
            ...
            // The token can also trip between the delay completing and this check,
            // so the flag is still worth testing after an uncancelled delay.
        ...
        /// The property name. Defaults to the caller's member name.
        /// </param>
